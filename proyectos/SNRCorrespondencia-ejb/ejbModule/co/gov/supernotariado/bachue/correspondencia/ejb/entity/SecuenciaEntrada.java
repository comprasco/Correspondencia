...
/**
 * Entidad para mapear la tabla SDB_PNG_CORRESPONDENCIA_SECUENCIA_ENTRADA
 *
 */
@Entity
@Table(name="SDB_PNG_CORRESPONDENCIA_SECUENCIA_ENTRADA")
@NamedQueries({
	@NamedQuery(name = SecuenciaEntrada.SECUENCIAENTRADA_POR_ENTRADA, query = "SELECT t FROM SecuenciaEntrada t WHERE t.entrada.id = ?1 ORDER BY t.id"),
	@NamedQuery(name = SecuenciaEntrada.SECUENCIAENTRADA_POR_IDENTIFICADOR_ORDEN_IDENTRADA_DESC_ID_ASC, query = "SELECT t FROM SecuenciaEntrada t WHERE t.identificador = ?1 ORDER BY t.entrada.id DESC, t.id ASC")
})
public class SecuenciaEntrada implements EntidadGenerica {
	
	/** Constante nombre consulta */
	public static final String SECUENCIAENTRADA_POR_ENTRADA = "SECUENCIAENTRADA_POR_ENTRADA";

	/** Constante nombre consulta */
	public static final String SECUENCIAENTRADA_POR_IDENTIFICADOR_ORDEN_IDENTRADA_DESC_ID_ASC = "SECUENCIAENTRADA_POR_IDENTIFICADOR_ORDEN_IDENTRADA_DESC_ID_ASC";

	/** Identificador de la tabla */
	@Id
	@SequenceGenerator(name = "SECUENCIA_SECUENCIA_ENTRADA", sequenceName = "SEC_PNG_CORRESPONDENCIA_SECUENCIA_ENTRADA", allocationSize=1)
	@GeneratedValue(strategy = GenerationType.SEQUENCE, generator = "SECUENCIA_SECUENCIA_ENTRADA")
	private long id;

	/**
	 * Entrada asociada a la secuencia
	 */
	@ManyToOne
